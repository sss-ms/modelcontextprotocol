---
title: 调试
description: Model Context Protocol (MCP) 集成的综合调试指南
---
...
1. **MCP Inspector**
   - 交互式调试界面
   - 直接服务器测试
   - 详见 [Inspector 指南](/docs/tools/inspector)
...
在 Claude Desktop 中访问 Chrome 的开发者工具来调查客户端错误：
...
MCP 服务器仅自动继承部分环境变量，如 `USER`、`HOME` 和 `PATH`。
