...
- **客户端** 在宿主应用程序内与服务器保持 1:1 连接
- **服务器** 向客户端提供上下文、工具和提示

```mermaid
flowchart LR
    subgraph "&nbsp;宿主 (例如 Claude Desktop)&nbsp;"
        client1[MCP Client]
        client2[MCP Client]
    end
    subgraph "服务器进程"
...
2. **HTTP 与 SSE 传输**
   - 使用 Server-Sent Events 进行服务器到客户端的消息传输
   - 使用 HTTP POST 进行客户端到服务器的消息传输

所有传输都使用 [JSON-RPC](https://www.jsonrpc.org/) 2.0 交换消息。有关 Model Context Protocol 消息格式的详细信息，请参阅[规范](https://spec.modelcontextprotocol.io)。
